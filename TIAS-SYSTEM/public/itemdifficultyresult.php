...
<div>
        ...
        <div class="panel panel-info">
              <div class="panel-heading">
                <h3 class="panel-title"><LI> DIFICULTY INDEX RESULT OF <?php echo $quiz['exam_title']; ?></h3>
              </div>
  ...
  <?php 
    ...
    foreach($final_quesDU as $k=>$di){
      ...
      foreach($q_ans as $p=>$v){
       ...
       ?>
       <tr class="grp_<?php echo ($k);?>"><th   style="width:100px;">Item No.</th><th style="width:100px;">Group Criterion(27%)</th><th style="width:70px;"><?php echo ($correctAnsIndex == 0)?'<span style="color:#ff0000;">✓A</span>':'<span style="color:#000;">A</span>';?></th>
      <th style="width:70px;"><?php echo ($correctAnsIndex == 1)?'<span style="color:#ff0000;">✓B</span>':'<span style="color:#000;">B</span>';?></th><th style="width:70px;"><?php echo ($correctAnsIndex == 2)?'<span style="color:#ff0000;">✓C</span>':'<span style="color:#000;">C</span>';?></th>
      <th style="width:70px;"><?php echo ($correctAnsIndex == 3)?'<span style="color:#ff0000;">✓D</span>':'<span style="color:#000;">D</span>';?></th>
       <th style="width:70px;"><?php echo ($correctAnsIndex == 4)?'<span style="color:#ff0000;">✓E</span>':'<span style="color:#000;">E</span>';?></th>


      </th><th style="width:70px;">NA</th><th style="width:70px;">Difficulty Index</th><th >Remarks</th><th >Action</th></tr>
     <tr class="grp_<?php echo ($k);?>"><td rowspan="2"><?php echo ($i);?></td><td >Upper Group(<?php echo $groupCnt;?>)</td><td><?php echo $di[0];?></td><td><?php echo $di[1]; ?></td><td><?php echo $di[2];?></td><td><?php echo $di[3]; ?></td><td><?php echo $di[4];?></td><td><?php echo $di[5];?></td><td rowspan="2"><?php echo $diffIndex = round(($final_quesDU[$k][$correctAnsIndex] + $final_quesDL[$k][$correctAnsIndex])/($groupCnt*2),3); ?></td><td rowspan="2"><?php echo range_difficulty($diffIndex);?></td> <td rowspan="2"> &nbsp;<a target="_blank" class="btn btn-primary btn-xs" href="edit-question.php?question_id=<?php echo $k;?>">Revise</a> &nbsp;<a  data-toggle="modal" data-target="#confirm-delete-modal1"  class="btn btn-danger btn-xs" onclick="sendId1(<?php echo $k;?>);" style="cursor:pointer;" >Discard</a> &nbsp;<a  data-toggle="modal" data-target="#confirm-delete-modal"  class="btn btn-success btn-xs" onclick="sendId(<?php echo $k;?>);" style="cursor:pointer;" >Retain</a> &nbsp;</td></tr>
     <tr class="grp_<?php echo ($k);?>"><td >Lower Group(<?php echo $groupCnt;?>)</td><td><?php echo $final_quesDL[$k][0];?></td><td><?php echo $final_quesDL[$k][1]; ?></td><td><?php echo $final_quesDL[$k][2];?></td><td><?php echo $final_quesDL[$k][3]; ?></td><td><?php echo $final_quesDL[$k][4];?></td><td><?php echo $final_quesDL[$k][5];?></td></tr> 
    
  <?php 
      $i++;
      }?>
      <tr><td colspan="11">*NA stands for Not Attempted i.e. selected none of the 5 options</td></tr>
      </table>
